...
interface OwnerDashboardData {
  ...
  stats: {
    ...
    likesTrend: string;
    savesTrend: string;
    deliveryTapsTrend: string;
    viewsUp: boolean;
    likesUp: boolean;
    savesUp: boolean;
    deliveryTapsUp: boolean;
  };
  insights: { title: string; why: string; action: string; href: string; accent: string }[];
}
...
export default function OwnerDashboard() {
  ...
  const submitClaimMutation = useMutation({
    mutationFn: async (claimData: {
      ...
      if (!res.ok) {
        ...
        throw new Error((body as any)?.message ?? "Failed to submit claim");
      }
      return res.json();
    },
    onSuccess: () => {
  ...
  const stats = data?.stats || { views: 0, uniqueViewers: 0, likes: 0, saves: 0, deliveryTaps: 0, viewsTrend: "0%", likesTrend: "0%", savesTrend: "0%", deliveryTapsTrend: "0%", viewsUp: true, likesUp: true, savesUp: true, deliveryTapsUp: true };
      ...
      {restaurant ? (
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4" data-testid="section-owner-stats">
            {[
              { icon: Eye, label: "Views", value: stats.views, color: "text-blue-500", bg: "bg-blue-50", trend: stats.viewsTrend, up: stats.viewsUp },
              { icon: Heart, label: "Likes", value: stats.likes, color: "text-rose-500", bg: "bg-rose-50", trend: stats.likesTrend, up: stats.likesUp },
              { icon: Bookmark, label: "Saves", value: stats.saves, color: "text-amber-500", bg: "bg-amber-50", trend: stats.savesTrend, up: stats.savesUp },
              { icon: ExternalLink, label: "Delivery Taps", value: stats.deliveryTaps, color: "text-[#00B14F]", bg: "bg-[#00B14F]/10", trend: stats.deliveryTapsTrend, up: stats.deliveryTapsUp },
            ].map(({ icon: Icon, label, value, color, bg, trend, up }) => (
              <div key={label} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5" data-testid={`stat-${label.toLowerCase().replace(/\s+/g, "-")}`}>
                ...
                <div className={`flex items-center gap-0.5 mt-1 text-xs ${up ? "text-[#00B14F]" : "text-red-400"}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="p-4 rounded-xl bg-[#00B14F]/5 border border-[#00B14F]/10">
                ...
                <p className="text-xl font-bold text-gray-800">
                  {stats.views > 0 ? Math.round((stats.likes / stats.views) * 100) : 0}%
                </p>
                <p className="text-[11px] text-gray-400 mt-0.5">Likes per View</p>
              </div>
              <div className="p-4 rounded-xl bg-[var(--admin-blue-10)] border border-[var(--admin-blue-10)]">
                ...
                <p className="text-xl font-bold text-gray-800">{stats.uniqueViewers ?? stats.views}</p>
                <p className="text-[11px] text-gray-400 mt-0.5">Unique users who viewed</p>
              </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6" data-testid="section-top-insights">
            ...
            {insights.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-8 text-center text-muted-foreground gap-2">
                ...
                <span className="text-xs">Insights will appear as activity data accumulates</span>
              </div>
            ) : (
              <div className="space-y-3">
        ...
        <>
          ...
          {claims.some((c: any) => c.status === "pending") ? (
            <div className="bg-amber-50 border border-amber-100 rounded-2xl p-8 text-center" data-testid="card-claim-pending">
              <div className="w-16 h-16 rounded-2xl bg-amber-100 flex items-center justify-center mx-auto mb-4">
                <Store className="w-8 h-8 text-amber-500" />
              </div>
              <h3 className="text-lg font-semibold text-gray-800 mb-1">Claim pending review</h3>
              <p className="text-sm text-gray-500 max-w-md mx-auto">Your restaurant claim has been submitted and is awaiting admin approval. You'll see your dashboard data once approved.</p>
            </div>
          ) : (
