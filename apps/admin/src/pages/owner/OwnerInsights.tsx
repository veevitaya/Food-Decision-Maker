...
import { Lightbulb, Eye, Heart, ExternalLink, Users, Target, ArrowUp, ArrowDown, Check, Loader2 } from "lucide-react";
...
export default function OwnerInsights() {
  ...
  const EMPTY_METRICS = [
    { label: "Impressions", value: "—", trend: "–", up: true, icon: Eye, desc: "Times your restaurant was shown" },
    { label: "Swipe Right Rate", value: "—", trend: "–", up: true, icon: Heart, desc: "Users who liked your listing" },
    { label: "Clickout Rate", value: "—", trend: "–", up: true, icon: ExternalLink, desc: "Users who tapped to order" },
    { label: "Unique Viewers", value: "—", trend: "–", up: true, icon: Users, desc: "Saves / engaged users" },
  ];

  const liveMetrics = useMemo(() => {
    if (!insightsData?.overview) return EMPTY_METRICS;
  ...
  const liveCompetitor = useMemo(() => {
    if (!insightsData?.competitorBenchmark) return [];
  ...
  const [recommendations, setRecommendations] = useState<{ title: string; impact: "high" | "medium" | "low"; reason: string; action: string; completed: boolean }[]>([]);
  ...
  const liveAudience = insightsData?.audienceBreakdown ?? [];

  const baseScore = insightsData?.opportunityScore ?? 0;
  ...
  return (
    <div className="space-y-8" data-testid="owner-insights-page">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 p-6" data-testid="card-recommendations">
          ...
          <div className="space-y-2.5">
            {recommendations.length === 0 && (
              <p className="text-sm text-gray-400 text-center py-6">No recommendations available yet. Check back after more activity.</p>
            )}
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6" data-testid="card-audience">
            ...
            <div className="space-y-2.5">
              {liveAudience.length === 0 && (
                <p className="text-sm text-gray-400 text-center py-4">No audience data yet.</p>
              )}
              {liveAudience.map(a => (
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6" data-testid="card-competitor-comparison">
        ...
        {liveCompetitor.length === 0 && (
          <p className="text-sm text-gray-400 text-center py-6">Competitor data not available yet.</p>
        )}
