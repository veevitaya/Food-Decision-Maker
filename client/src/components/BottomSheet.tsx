...
export function BottomSheet({
  ...
  const pullDragStart = useRef<number | null>(null);
  ...
  const { data: newNearby = [], isLoading: isLoadingNearby } = useRestaurants({
    mode: "new",
  ...
  const getSheetHeight = useCallback(() => {
    return drawerRef.current?.offsetHeight ?? window.innerHeight * 0.82;
  }, []);

  const snapOpen = useCallback(() => {
    animate(y, 0, { type: "spring", damping: 22, stiffness: 200, mass: 1 });
    setIsOpen(true);
    onDrawerStateChange?.(true);
  }, [y, onDrawerStateChange]);

  const snapHidden = useCallback(() => {
    animate(y, getSheetHeight(), { type: "spring", damping: 22, stiffness: 200, mass: 1 });
    setIsOpen(false);
    onDrawerStateChange?.(false);
  }, [y, getSheetHeight, onDrawerStateChange]);

  useEffect(() => {
    if (forceOpen && !isOpen) snapOpen();
  }, [forceOpen, isOpen, snapOpen]);
  ...
  const handleDragEnd = useCallback(
    (_: unknown, info: { offset: { y: number }; velocity: { y: number } }) => {
      ...
      if (velocity > 300 || currentY > height * 0.3) {
        snapHidden();
      } else {
        snapOpen();
      }
    },
    [y, getSheetHeight, snapOpen, snapHidden],
  );

  // Pull-tab drag handlers: user drags up from the hidden tab to reopen the sheet
  const handlePullDown = useCallback((e: React.PointerEvent) => {
    pullDragStart.current = e.clientY;
    e.currentTarget.setPointerCapture(e.pointerId);
  }, []);

  const handlePullMove = useCallback(
    (e: React.PointerEvent) => {
      if (pullDragStart.current === null) return;
      const height = getSheetHeight();
      const delta = e.clientY - pullDragStart.current; // negative = moving up
      const newY = Math.max(0, Math.min(height, height + delta));
      y.set(newY);
    },
    [y, getSheetHeight],
  );
  ...
  const handlePullUp = useCallback(
    (e: React.PointerEvent) => {
      if (pullDragStart.current === null) return;
      const height = getSheetHeight();
      const delta = e.clientY - pullDragStart.current;
      pullDragStart.current = null;
      if (delta < -height * 0.2) {
        snapOpen();
      } else {
        animate(y, height, { type: "spring", damping: 22, stiffness: 200 });
      }
    },
    [y, getSheetHeight, snapOpen],
  );
  ...
  return (
    <>
      ...
      >
        ...
        {/* Scrollable content — pointer events here do NOT start a drag */}
        <div
        ...
        >
          {isSearching ? (
                  ...
                  {searchResults.map((r, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {!isNameMatch && (
                          <span className="text-[9px] text-muted-foreground/60 bg-gray-100 px-2 py-0.5 rounded-full flex-shrink-0">
                            Similar
                          </span>
                        )}
            ...
            <div className="px-5 pt-2 pb-5">
              <p
                className="text-[22px] font-bold text-foreground tracking-tight leading-tight mb-1"
                data-testid="text-search-heading"
              >
                ...
                {suggestions.slice(0, 8).map((r) => (
                  ...
                  >
                    <div
                      ...
                      style={{
                        background:
                          "linear-gradient(135deg, hsl(40,50%,96%) 0%, hsl(35,40%,91%) 100%)",
                      }}
                    >
                      {r.category?.includes("Thai")
                        ? "🇹🇭"
                        : r.category?.includes("Japan")
                          ? "🇯🇵"
                          : r.category?.includes("Korean")
                            ? "🇰🇷"
                            : r.category?.includes("Italian") || r.category?.includes("Pizza")
                              ? "🍕"
                              : r.category?.includes("Burger")
                                ? "🍔"
                                : r.category?.includes("Ramen") || r.category?.includes("Noodle")
                                  ? "🍜"
                                  : r.category?.includes("Seafood")
                                    ? "🦐"
                                    : r.category?.includes("Cocktail") ||
                                        r.category?.includes("Bar") ||
                                        r.category?.includes("Rooftop")
                                      ? "🍸"
                                      : r.category?.includes("Coffee") ||
                                          r.category?.includes("Cafe")
                                        ? "☕"
                                        : r.category?.includes("Dessert") ||
                                            r.category?.includes("Ice")
                                          ? "🍰"
                                          : "🍽️"}
                    </div>
                    <div className="flex-1 min-w-0">
                    ...
                    <span className="text-[9px] text-muted-foreground/50 bg-gray-50 px-2 py-0.5 rounded-full flex-shrink-0">
                      {Number(r.trendingScore) >= 90
                        ? "🔥 Hot"
                        : Number(r.trendingScore) >= 80
                          ? "📈 Trending"
                          : "✨ Try"}
                    </span>
                    <span className="text-muted-foreground/40 text-xs">›</span>
            ...
            <>
              <div className="px-6 pt-1 pb-4">
                <h2
                  className="text-[22px] font-bold text-foreground mb-5 flex items-baseline gap-1.5 flex-wrap"
                  data-testid="text-let-toast"
                >
                  Let{" "}
                  <img
                    src={toastLogoPath}
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                  <button
                    ...
                    className="flex items-center gap-3 py-4 px-5 rounded-[20px] transition-all duration-200 active:scale-[0.97] relative overflow-hidden gpu-accelerated bg-white dark:bg-card border border-gray-100/80 dark:border-border"
                    style={{
                      boxShadow: !isGroup
                      ...
                      {moreVibes.length > 0 && (
                        <div className="grid grid-cols-1 mt-1">
                          ...
                          >
                            ...
                            <span className="text-[11px] text-muted-foreground bg-gray-200/60 px-2 py-0.5 rounded-full ml-1">
                              {moreVibes.length}
                            </span>
                          </button>
                        </div>
                    ...
                    >
                      ...
                      >
                        ...
                        <p className="text-[12px] text-muted-foreground mb-4">
                          Use these more often and they'll move to your main vibes
                        </p>
              ...
              <div className="px-6 mt-6 mb-4">
                <button
                  ...
                  style={{
                    background:
                      "linear-gradient(135deg, hsl(40,50%,96%) 0%, hsl(35,40%,93%) 50%, hsl(30,35%,91%) 100%)",
                    boxShadow:
                      "0 4px 20px -4px rgba(0,0,0,0.08), inset 0 1px 0 rgba(255,255,255,0.8)",
                  }}
                >
                  ...
                  <p className="text-[16px] font-semibold text-foreground tracking-tight leading-snug pr-10">
                    Let Toast pick for you
                  </p>
      ...
      {/* Pull-up tab — appears at the very bottom when the sheet is hidden */}
      <AnimatePresence>
        {!isOpen && (
          <motion.div
            initial={{ y: 80 }}
            ...
            className="absolute bottom-0 left-0 right-0 z-[45] flex justify-center"
            style={{ touchAction: "none" }}
            onPointerDown={handlePullDown}
            onPointerMove={handlePullMove}
            onPointerUp={handlePullUp}
            onPointerCancel={handlePullUp}
          >
            <div
              className="flex flex-col items-center gap-1.5 bg-white/95 backdrop-blur-md rounded-t-2xl px-10 py-3 cursor-grab active:cursor-grabbing select-none"
              style={{ boxShadow: "0 -4px 24px -4px rgba(0,0,0,0.14)" }}
            >
              <div className="w-10 h-[5px] bg-gray-300 rounded-full" />
              <span className="text-[11px] text-muted-foreground font-medium">Drag up to open</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
