...
function OwnerDashboard() {
  ...
  const [campaignForm, setCampaignForm] = useState<Omit<Campaign, "id" | "status">>({
    title: "",
    dealType: "percentage",
    dealValue: "",
    description: "",
      ...
      <div className="mb-4">
        <div className="bg-white dark:bg-card rounded-2xl overflow-hidden border border-gray-100 dark:border-border">
          <div className="px-5 py-4">
            <p className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-widest mb-3">Your Restaurant</p>
            <input
              type="text"
              value={ownerProfile.restaurantName}
              onChange={(e) => updateOwner({ restaurantName: e.target.value })}
              placeholder="Restaurant name"
              className="w-full text-[17px] font-bold bg-transparent border-none outline-none placeholder:text-muted-foreground/30 tracking-tight mb-3"
            ...
            <div className="flex gap-2">
              <input
                ...
                onChange={(e) => updateOwner({ category: e.target.value })}
                placeholder="Category (e.g. Thai)"
                ...
                data-testid="input-restaurant-category"
              />
              <input
                type="text"
          ...
          <button
            onClick={() => {
              setCampaignForm({
                ...
                startDate: "", endDate: "", conditions: [], minSpend: "", maxRedemptions: "",
              });
            ...
            <div key={campaign.id} className="bg-white dark:bg-card rounded-2xl border border-gray-100 dark:border-border overflow-hidden mb-2.5">
              <div className="px-5 py-4 flex items-center gap-4">
                <div className={`w-11 h-11 rounded-2xl flex items-center justify-center ${
                  isActive ? "bg-green-50" : isPaused ? "bg-amber-50" : isEnded ? "bg-gray-50" : "bg-blue-50"
                }`}>
                  <Tag className={`w-5 h-5 ${
                    isActive ? "text-green-500" : isPaused ? "text-amber-500" : isEnded ? "text-gray-400" : "text-blue-500"
                  }`} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => {
                        ...
                        const updated = (ownerProfile.campaigns || []).map(c =>
                          c.id === campaign.id ? { ...c, status: newStatus as Campaign["status"] } : c
                        );
          ...
          {showCampaignForm && (
            ...
            >
              <motion.div
                ...
                exit={{ y: "100%" }}
                transition={{ type: "spring", damping: 22, stiffness: 200, mass: 1 }}
                className="w-full max-w-md bg-white dark:bg-card rounded-t-[28px] p-6 pb-10 max-h-[90vh] overflow-y-auto"
                style={{ boxShadow: "0 -10px 40px rgba(0,0,0,0.12)" }}
                onClick={(e) => e.stopPropagation()}
              >
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-widest mb-1.5 block">Timeframe</label>
                    <div className="flex gap-2">
                      <div className="flex-1">
                        ...
                        <input
                          ...
                          className="w-full px-3 py-2.5 rounded-xl bg-gray-50 dark:bg-muted text-sm outline-none"
