...
export default function RestaurantDetail() {
  ...
  const handleGrabOrder = () => {
    const grabDeepLink = `grab://food/search?q=${encodeURIComponent(restaurant.name)}`;
    const grabWebFallback = `https://food.grab.com/th/en/restaurants?search=${encodeURIComponent(restaurant.name)}`;

    const newWindow = window.open("", "_blank");

    const iframe = document.createElement("iframe");
    iframe.style.display = "none";
    iframe.src = grabDeepLink;
    document.body.appendChild(iframe);

    setTimeout(() => {
      document.body.removeChild(iframe);
      if (newWindow && !newWindow.closed) {
        newWindow.location.href = grabWebFallback;
      }
    }, 1500);
  };
    ...
    <div className="w-full min-h-[100dvh] bg-white pb-40" data-testid="restaurant-detail-page">
      ...
      <div className="fixed left-0 right-0 bg-white/95 backdrop-blur-md border-t border-gray-100/80 px-6 py-3 z-50 flex gap-3" style={{ bottom: "calc(52px + max(env(safe-area-inset-bottom, 0px), 16px))" }}>
        <button
          ...
          data-testid="button-order-grab"
          className="flex-1 py-3.5 rounded-full bg-[#00B14F] text-white font-bold text-sm flex items-center justify-center gap-2 active:scale-[0.97] transition-transform duration-200"
          style={{ boxShadow: "0 4px 15px -3px rgba(0,177,79,0.3)" }}
        >
          🛵 Order on Grab
        </button>
        <button
