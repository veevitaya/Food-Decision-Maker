...
function getTimeContext(): { meal: string; emoji: string; greeting: string } {
  const hour = new Date().getHours();
  if (hour >= 5 && hour < 11) return { meal: "breakfast", emoji: "🌅", greeting: "Good morning" };
  if (hour >= 11 && hour < 14) return { meal: "lunch", emoji: "☀️", greeting: "Lunchtime" };
  if (hour >= 14 && hour < 17) return { meal: "afternoon snack", emoji: "🍵", greeting: "Afternoon" };
  if (hour >= 17 && hour < 21) return { meal: "dinner", emoji: "🌆", greeting: "Evening" };
  return { meal: "late night bite", emoji: "🌙", greeting: "Late night" };
}
      ...
      >
        <div className="relative h-44 overflow-hidden">
          ...
          <div className="absolute bottom-3 left-4 right-4">
            <h3 className="text-white font-semibold text-lg tracking-tight drop-shadow-sm" data-testid={`text-pick-name-${restaurant.id}`}>{restaurant.name}</h3>
            <p className="text-white/80 text-xs mt-0.5" data-testid={`text-pick-category-${restaurant.id}`}>{restaurant.category}</p>
          </div>
        </div>
          ...
          <AnimatePresence mode="wait">
            {phase === "thinking" ? (
              ...
              >
                ...
                >
                  <p className="text-[11px] font-bold text-muted-foreground/50 mb-2">{timeContext.emoji} {timeContext.greeting}</p>
