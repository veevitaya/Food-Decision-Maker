...
function getBannerPerformance(banner: BannerRecord) {
  const impressions = Math.max(0, banner.impressions ?? 0);
  const clicks = Math.max(0, banner.clicks ?? 0);
  const conversions = Math.round(clicks * 0.1);
  const revenue = conversions * 180;
  const dailyBudget = 1000;
  const totalBudget = 30000;
  const spent = Math.round((impressions / Math.max(impressions, 20000)) * totalBudget);
  const format = adFormats[banner.id % adFormats.length].value;
  const variant = banner.id % 3 === 0 ? "B" : "A";
  const targeting = {
    age: ageRanges[banner.id % (ageRanges.length - 1)].label,
    gender: banner.id % 2 === 0 ? "All" : "Female",
    userType: userTypeOptions[banner.id % userTypeOptions.length],
  };
...
export default function AdminBanners() {
  ...
  const { data: banners = [], isLoading } = useQuery<BannerRecord[]>({
  ...
  return (
    <div data-testid="admin-banners-page" className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {banners.map((banner) => {
            const perf = getBannerPerformance(banner);
            const ctr =
              banner.impressions && banner.impressions > 0
                ? ((banner.clicks || 0) / banner.impressions * 100).toFixed(1)
                : ((perf.clicks / perf.impressions) * 100).toFixed(1);
            const spentPct = Math.min(100, Math.round((perf.spent / perf.totalBudget) * 100));
            const fmtColors = formatBadgeColors[perf.format] || formatBadgeColors.display_banner;
                ...
                {banner.imageUrl && (
                  <div className="aspect-video rounded-t-2xl overflow-hidden bg-gray-50 dark:bg-muted relative">
                    ...
                    <div className="absolute top-3 left-3 flex items-center gap-2">
                      ...
                      >
                        {formatLabels[perf.format] || "Display"}
                      </span>
                      {perf.variant === "B" && (
                        <span
                ...
                <div className="p-5 space-y-3">
                  ...
                  <div className="flex items-center gap-1 text-xs text-muted-foreground" data-testid={`revenue-${banner.id}`}>
                    ...
                    <span className="font-semibold text-foreground">
                      {perf.revenue.toLocaleString()} THB
                    </span>
                  </div>

                  <div data-testid={`budget-bar-${banner.id}`}>
                    <div className="flex items-center justify-between text-[10px] text-muted-foreground mb-1">
                      <span>Budget: {perf.spent.toLocaleString()} / {perf.totalBudget.toLocaleString()} THB</span>
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap" data-testid={`targeting-pills-${banner.id}`}>
                    <span className="inline-flex items-center rounded-full bg-gray-100 dark:bg-muted text-foreground px-2.5 py-0.5 text-[10px] font-medium">
                      Age {perf.targeting.age}
                    </span>
                    <span className="inline-flex items-center rounded-full bg-gray-100 dark:bg-muted text-foreground px-2.5 py-0.5 text-[10px] font-medium">
                      {perf.targeting.gender}
                    </span>
                    <span className="inline-flex items-center rounded-full bg-gray-100 dark:bg-muted text-foreground px-2.5 py-0.5 text-[10px] font-medium">
                      {perf.targeting.userType}
                    </span>
                  </div>
