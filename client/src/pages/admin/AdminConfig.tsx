...
type SerializedFeatureToggle = Omit<FeatureToggle, "icon">;
type SerializedApiIntegration = Omit<ApiIntegration, "icon">;

type AdminConfigPayload = {
  features?: Record<string, boolean>;
  vibes?: Record<string, boolean>;
  apiStatus?: Record<string, ApiIntegration["status"]>;
  uiConfig?: Partial<UIConfig>;
  imageUrls?: Record<string, string>;
  featuresList?: SerializedFeatureToggle[];
  apisList?: SerializedApiIntegration[];
  imagesList?: AppImage[];
  vibesList?: VibeConfig[];
};
...
const FEATURE_ICON_BY_ID: Record<string, typeof Settings2> = Object.fromEntries(
  DEFAULT_FEATURES.map((feature) => [feature.id, feature.icon]),
);

const API_ICON_BY_ID: Record<string, typeof Key> = Object.fromEntries(
  DEFAULT_APIS.map((api) => [api.id, api.icon]),
);
...
function cloneConfigState(state: AdminConfigState): AdminConfigState {
  return {
    features: state.features.map((item) => ({ ...item })),
    images: state.images.map((item) => ({ ...item })),
    vibes: state.vibes.map((item) => ({ ...item })),
    uiConfig: {
      ...state.uiConfig,
      bottomNavLabels: { ...state.uiConfig.bottomNavLabels },
    },
    apis: state.apis.map((item) => ({ ...item })),
  };
}

function buildConfigState(payload?: AdminConfigPayload): AdminConfigState {
  let nextFeatures = DEFAULT_FEATURES.map((item) => ({ ...item }));
  let nextImages = DEFAULT_IMAGES.map((item) => ({ ...item }));
  let nextVibes = DEFAULT_VIBES.map((item) => ({ ...item }));
  let nextUiConfig = {
    ...DEFAULT_UI,
    bottomNavLabels: { ...DEFAULT_UI.bottomNavLabels },
  };
  let nextApis = DEFAULT_APIS.map((item) => ({ ...item }));

  if (payload?.features) {
    nextFeatures = nextFeatures.map((feature) => ({
      ...feature,
      enabled: payload.features?.[feature.id] ?? feature.enabled,
    }));
  }
  if (payload?.featuresList && payload.featuresList.length > 0) {
    nextFeatures = payload.featuresList.map((feature) => ({
      ...feature,
      icon: FEATURE_ICON_BY_ID[feature.id] ?? Settings2,
    }));
  }

  if (payload?.vibes) {
    nextVibes = nextVibes.map((vibe) => ({
      ...vibe,
      enabled: payload.vibes?.[vibe.mode] ?? vibe.enabled,
    }));
  }
  if (payload?.vibesList && payload.vibesList.length > 0) {
    nextVibes = payload.vibesList.map((vibe) => ({ ...vibe }));
  }

  if (payload?.apiStatus) {
    nextApis = nextApis.map((api) => ({
      ...api,
      status: payload.apiStatus?.[api.id] ?? api.status,
    }));
  }
  if (payload?.apisList && payload.apisList.length > 0) {
    nextApis = payload.apisList.map((api) => ({
      ...api,
      icon: API_ICON_BY_ID[api.id] ?? Key,
    }));
  }
  ...
  if (payload?.uiConfig) {
    nextUiConfig = {
      ...
      ...payload.uiConfig,
      bottomNavLabels: {
        ...nextUiConfig.bottomNavLabels,
        ...(payload.uiConfig.bottomNavLabels ?? {}),
      },
    };
  }
  ...
  if (payload?.imageUrls) {
    nextImages = nextImages.map((image) => ({
      ...image,
      currentUrl: payload.imageUrls?.[image.id] ?? image.currentUrl,
    }));
  }
  if (payload?.imagesList && payload.imagesList.length > 0) {
    nextImages = payload.imagesList.map((image) => ({ ...image }));
  }
...
export default function AdminConfig() {
  ...
  const [initialConfig, setInitialConfig] = useState<AdminConfigState | null>(null);
  ...
  useEffect(() => {
    ...
    const loadConfig = async () => {
      try {
        ...
        const data = res.ok ? ((await res.json()) as AdminConfigPayload) : undefined;
        if (cancelled) return;
        const nextState = buildConfigState(data);
        setFeatures(nextState.features);
        setImages(nextState.images);
        setVibes(nextState.vibes);
        setUiConfig(nextState.uiConfig);
        setApis(nextState.apis);
        setInitialConfig(cloneConfigState(nextState));
      } catch {
        if (cancelled) return;
        const fallbackState = buildConfigState();
        setInitialConfig(cloneConfigState(fallbackState));
      } finally {
  ...
  const handleSave = async () => {
    const payload: AdminConfigPayload = {
      ...
      vibes: Object.fromEntries(vibes.map((v) => [v.mode, v.enabled])),
      apiStatus: Object.fromEntries(apis.map((a) => [a.id, a.status])),
      uiConfig,
      imageUrls: Object.fromEntries(images.map((image) => [image.id, image.currentUrl])),
    ...
    await apiRequest("PUT", "/api/admin/config", payload);
    setInitialConfig(
      cloneConfigState({
        features,
        images,
  ...
  const handleReset = () => {
    ...
    const snapshot = cloneConfigState(initialConfig);
    setFeatures(snapshot.features);
    setImages(snapshot.images);
    setVibes(snapshot.vibes);
    setUiConfig(snapshot.uiConfig);
    setApis(snapshot.apis);
    setHasChanges(false);
    setPendingChanges([]);
  };
  ...
  const handleFetchPlaces = async () => {
    ...
    try {
      ...
      const msg = typeof err?.message === "string" ? err.message : "Failed to fetch from Google Places API";
      setFetchError(msg);
    } finally {
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-6" data-testid="preview-modal">
          <div className="bg-white dark:bg-card rounded-3xl border border-gray-100 dark:border-border w-full max-w-2xl max-h-[80vh] overflow-hidden shadow-2xl">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100 dark:border-border bg-gray-50 dark:bg-muted">
              ...
              <button
                onClick={() => { void handleSave(); setShowPreview(false); }}
