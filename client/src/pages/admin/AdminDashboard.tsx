...
function DeliveryRing({ data }: { data: DashboardDetails["deliveryAttribution"] }) {
  const totalRaw = data.reduce((sum, d) => sum + d.clicks, 0);
  ...
  return (
    <svg width="110" height="110" viewBox="0 0 110 110" className="flex-shrink-0">
      ...
      <text x={cx} y={cy - 4} textAnchor="middle" className="fill-foreground text-[14px] font-bold">{totalRaw.toLocaleString()}</text>
...
export default function AdminDashboard() {
  ...
  const conversionFunnel = details?.conversionFunnel ?? [];
  const deliveryAttribution = details?.deliveryAttribution ?? [];
  const topRestaurants = details?.topRestaurants ?? [];
  const geoHotspots = details?.geoHotspots ?? [];
  const trendingCuisines = details?.trendingCuisines ?? [];

  const maxRestaurantSwipes = Math.max(...topRestaurants.map((r) => r.swipes), 1);
  const maxGeoOrders = Math.max(...geoHotspots.map((s) => s.orders), 1);
  ...
  const kpis = [
    ...
    },
    {
      ...
      value: details?.deliveryClicks ?? 0,
      icon: Truck,
      delta: "+18%",
      deltaUp: true,
