...
async function buildAnalyticsEvents(): Promise<AnalyticsEventRecord[]> {
  const logs = await storage.listPlacesRequestLogs(200);
  return logs.map((log) => ({
    id: log.id,
...
export async function registerRoutes(
  ...
  app.get("/api/admin/me", (req, res) => {
    if (req.session.isAdmin) return res.json({ isAdmin: true, username: process.env.ADMIN_EMAIL ?? "admin", role: "admin" });
  ...
  app.get("/api/analytics/user-segments", async (req, res) => {
    try {
      ...
      res.json([
        { id: "power", name: "Power Users", description: "Highly active users", estimatedCount: Math.round(total * 0.13) },
        { id: "new", name: "New Users", description: "Joined recently", estimatedCount: Math.round(total * 0.22) },
        { id: "thai", name: "Thai Food Lovers", description: "Prefer Thai cuisine", estimatedCount: Math.round(total * 0.28) },
        { id: "budget", name: "Budget Diners", description: "Price-conscious", estimatedCount: Math.round(total * 0.18) },
      ]);
    } catch {
      res.status(500).json({ message: "Internal server error" });
    }
    ...
    try {
      ...
      const top = allRestaurants
        ...
        .map((item) => ({
          ...
          name: item.name,
          count: Math.max(1, (item.trendingScore ?? 0) * 10),
        }));
      res.json(top);
    } catch {
      res.status(500).json({ message: "Internal server error" });
  ...
  app.get("/api/analytics/summary", async (req, res) => {
    try {
      ...
      const totalSwipes = (eventBreakdown.swipe_left ?? 0) + (eventBreakdown.swipe_right ?? 0);
